...
class Helper
{
    ...
    {
        $price_rupiah = "L. " . number_format($price, 2, '.', ',');
        return $price_rupiah;
    }
    ...
    {
        return Carbon::parse($date)->isoFormat('D-MM-YYYY');
    }
    ...
    public static function getDateDifferenceWithPlural($check_in, $check_out)
    {
        $day = Helper::getDateDifference($check_in, $check_out);
        $dayName = $day == 1 ? 'Dia' : 'Dias';
        $plural = Str::plural($dayName, $day);
        return $day . ' ' . $plural;
    }
